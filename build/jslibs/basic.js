...
if (!HTMLElement.prototype.hasOwnProperty("innerText")) {
		HTMLElement.prototype.__defineSetter__("innerText",function(v){return this.textContent = v});
...
function Scrollable(element) {
    // need scrollbars
    // scroll with mouse, mousewheel, touch, automatically by placing cursor
    // scrollbar moves in response to a direct change of scroll-- properties
    //    can be done in setters (getters?) of scrollTop/scrollLeft
    // scrollbar moves/resizes in response to resize: population or style change
    //    
}
*/
function Scrollable(elem){
    var vq,hq,vmax,hmax,vold,hold,vstart,hstart;elem.scrollbars=[];
    if ((typeof elem.onwheel === "object") && (typeof elem.onmousemove === "object") && elem.extendable /*&& (getComputedStyle(elem).getPropertyValue("overflow") == "hidden")*/) {
        elem.appendChild(elem.vscroll = new Scroll("vertical"));
        elem.appendChild(elem.hscroll = new Scroll("horizontal"));
